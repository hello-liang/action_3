...
class videoRead(caffe.Layer):
  ...
  def initialize(self):
    ...
    self.flow = False
    self.buffer_size = test_buffer  #num videos processed per batch 3
    ...
    self.video_list = 'ucf101_split1_testVideos_check2.txt'  #do not have blank line????
  ...
  def setup(self, bottom, top):
    ...
    video_dict = {}
    current_line = 0
    self.video_order = []  #creat a list to process
    for ix, line in enumerate(f_lines):

      clc_video = line.split('\t')[0].split('/')[0]
      video = line.split('\t')[0].split('/')[1]  #video name #need to test
      l = int(line.split('\t')[1])      #class

      frames = glob.glob('%s%s/%s/*.jpg' %(self.path_to_images, clc_video, video)) # I think ,split video to frames
      num_frames = len(frames)
      video_dict[video] = {}   # creat a dict ,each dict have frame ,reshape,num_frames,label.
      video_dict[video]['frames'] = frames[0].split('/frame')[-2] + '/frame_%04d.jpg'
  ...
  def forward(self, bottom, top):
      ...
      if self.idx + self.buffer_size >= self.num_videos:    #0+3 >=12 , debug test1 at first . I think so 
        if (self.idx-self.num_videos < 0):
         ...
         idx_list = range(self.idx, self.idx+self.buffer_size)   #if self.idx=0 then this is [0,1,2]
 
            
      
      
      for j in idx_list:  #0,1,2
        anchor_idx = j #0
        ...
        if (num_frames < 100):
            ...
            jump_idx2 = 17
        index = random.randint(0, num_frames)#0 random choose a num at the length of videos
        flag = 0;
        for i in range(pair):  #pair=15 means each video get 15 pair.
          if flag:
             index = random.randint(0, num_frames)
             jump_idx = 0
             flag = 0
             
          idx = jump_idx + (index+1) #0+0+1
          idx2 = idx + 1  #2
          idx3 = idx + jump_idx2 # num_frames<100 is 7  if >100 is 17
         
          index += 1  #to next frame of this video 
          if (num_frames < 100):
            jump_idx += 2  # if numframes <100  jump_idx +2
          else:
            jump_idx += 7
          
          if (idx > num_frames-2):   # if  complate the loop  of video ,will random choose another frame and begin loop all video
                idx = num_frames-2
                flag = 1
          ...
          curr_frame = frames % idx  ###the first frame
          ...
          #consecutive frame
          next_frame = frames % idx2  # the second frame
          f = random.randint(0,1)
          im_flip.extend([f]) 
          ...
          #same video frame  # the third frame
          ...
          while (v_idx == anchor_idx):
             v_idx = random.randint(0, len(idx_list)-1) #  one of the batch  video ,batch is 3
             key2 = self.video_order[v_idx]  ###so must find another video as the anchor_idx
      ...
      #data is 45 = 3buffer  *15 pair
      #print("there!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")	
      #print(len(data))	
      #sys.exit("use_this_debug!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")	 
      ...
      for k in range(self.N):
            top[0].data[k,...] = data[k]    #top out the data 
