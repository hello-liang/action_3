...
%kmeans on heart disease dataset with c=4
...
g0=ones(n,1);
gIdx=zeros(n,1);
D=zeros(n,k);  %each sample distance to each class
P=zeros(n,k);
OP=zeros(1,k);
...
while any(g0~=gIdx) % until the classify will not change then the result is ok 
    ...
    [z,gIdx]=min(D,[],2); %the col vector which contain the min of each row
