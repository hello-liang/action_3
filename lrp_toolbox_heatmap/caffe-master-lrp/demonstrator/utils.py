...
def lrp_hm(net, input_images, lrp_method = 'epsilon', lrp_param = 0.0000001, target_class_inds = -1, switch_layer = -1, single_mode=False, verbose_output= False):
    ...
    for b_i, input_image_batch in enumerate(input_batches):
       ...
       # out = net.forward()
        out = net.forward(blobs=['fc8']) #out['prob']  ## 15.4199295,  -1.7109375, -13.436903  #### 1 .0000000e+00, 3.6321101e-08, 2.9351944e-13


        target_class = get_target_indices(target_class_inds, out['fc8']) ##out['prob']  #at here ,I think can use score ,yes,can !
    ...
    output = np.concatenate(output)  # this is a class
...
def load_model(model = 'caffenet', batchsize=16):
    ...
    if model == 'googlenet':
        ...
        net = caffe.Net('/media/liang/ssd/lrp_toolbox_heatmap/caffe-master-lrp/models/unsupervised/deploy.prototxt', #deploy_Lq_loss
                        '/media/liang/ssd/lrp_toolbox_heatmap/caffe-master-lrp/models/unsupervised/my_net.caffemodel',
                           caffe.TEST)  
        
    else:# ('bvlc_reference_caffenet' - default)
        net = caffe.Net('/media/liang/ssd/lrp_toolbox_heatmap/caffe-master-lrp/models/bvlc_reference_caffenet/deploy.prototxt',
