...
%r= randperm(size(feature_all,1));
%feature_all=feature_all(r,:);
%label_all=label_all(:,r);

%feature_all=feature_all(1:1000,:);
%label_all=label_all(:,1000);
...
for iteration=1:iterations
    ...
    %ASSIGN MEASURES with CLUSTERS     for 20 particles , make sure the
    %class  label represent  of each particles 
    for particle=1:particles
    ...
    [global_fitness, index] = max(swarm_fitness);       %GLOBAL BEST FITNESS
  ...
  fprintf('%3d. global fitness is %5.4f.  Acc :%5.4f.   Acc %5.4f\n',iteration,global_fitness,Acc,Acc);   
